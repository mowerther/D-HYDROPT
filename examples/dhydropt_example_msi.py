...
def plot_compact(Jm, support, vmax, fine_wl, band_wl, y, yhat0, yhatT, vals, out_path, title):
    """
    ...
    gs = fig.add_gridspec(
        2, 5,
        width_ratios=[1.1, 1.1, 0.05, 0.6, 1.6],
    ...
    def draw_heat(ax, J, name, xlim_range=(None, None), ylim_range=(None, None), show_xlabel=True):
        ...
        im = ax.imshow(A_plot, aspect='auto', cmap=cmap, norm=norm, interpolation='nearest',
                      extent=[float(fine_wl[0]), float(fine_wl[-1]), M - 0.5, -0.5], zorder=1)
    ...
    ax3.plot(band_wl, np.asarray(yhatT), 'o-', label='After gradient', zorder=3)
    ...
    ax3.set_xlabel('MSI band [nm]')
    ax3.set_ylabel('$R_{rs}[sr^{-1}]$')
    ax3.grid(True, alpha=0.3)
    ax3.legend(framealpha=0.9)

    ax4 = fig.add_subplot(gs[1, 4])
    ax4.plot(np.arange(len(vals)), np.asarray(vals), linewidth=2)
    ax4.set_xlabel("Optimisation step")
    ax4.set_ylabel("MSE")
    ...
    fig.text(center_x, 0.02, "Hydropt λ [nm]", ha='center', fontsize=10)
